...
--
-- ⛔ **不要单独重跑这个文件 —— 它会让公开榜单立刻白屏。** 下面写着「幂等,可重复跑」,
--    那句话在 0026 之后就不成立了:本文件会重建一个 security_invoker 视图并再次
--    `revoke select (project_*) … from anon`,而这正是 0026 必须存在的原因 ——
--    anon 会立刻在整张 leaderboard 上拿到 42501。本文件的意图已由 0026 + 0028 承接。
