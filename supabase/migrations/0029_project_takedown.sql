...
--
-- ⛔ **第 5 节已被 0030 撤销,不要单独重跑这个文件。** 那一节让下架/封禁去清空行上的
--    四个 project_* 列并删存储桶里的图 —— 是设计错误:可撤销的动作不该销毁不可恢复的
--    数据(客户端不会重传已 sync 过的图,见 0030 文件头)。重跑会把那两个触发器装回来。
--    第 1–4 节(黑名单表、private.is_project_blocked、触发器拒绝、视图抑制)是对的,仍然有效。
--
